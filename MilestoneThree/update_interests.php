...
function fetchUsersWithInterest($interestName, $userId, $conn) { $query = " SELECT u.id, u.username, i.interest_name FROM users u JOIN interests i ON u.id = i.user_id WHERE i.interest_name = '$interestName' AND u.id != '$userId' "; $result = mysqli_query($conn, $query); if (!$result) { return ['error' => 'Query Failed: ' . mysqli_error($conn)]; } $users = []; while ($row = mysqli_fetch_assoc($result)) { $users[] = $row; } return ['users' => $users]; }
